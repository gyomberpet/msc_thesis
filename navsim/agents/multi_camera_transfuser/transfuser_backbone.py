...
class TransfuserBackbone(nn.Module):
    ...
    def forward(self, front_image, back_image, lidar):
        """
        ...
        """
        front_image_features, back_image_features, lidar_features = front_image, back_image, lidar
        ...
        front_image_layers = iter(self.image_encoder.items())
        back_image_layers = iter(self.image_encoder.items())
        lidar_layers = iter(self.lidar_encoder.items())

        # Stem layer.
        # In some architectures the stem is not a return layer, so we need to skip it.
        if len(self.image_encoder.return_layers) > 4:
            front_image_features = self.forward_layer_block(front_image_layers, self.image_encoder.return_layers, front_image_features)
            back_image_features = self.forward_layer_block(back_image_layers, self.image_encoder.return_layers, back_image_features)
        ...
        for i in range(4):
            ...
            front_image_features, lidar_features = self.fuse_features(front_image_features, lidar_features, i)
            back_image_features, lidar_features = self.fuse_features(back_image_features, lidar_features, i)
        ...
        if self.config.transformer_decoder_join:
            ...
            front_image_features = self.global_pool_img(front_image_features)
            front_image_features = torch.flatten(front_image_features, 1)
            back_image_features = self.global_pool_img(back_image_features)
            back_image_features = torch.flatten(back_image_features, 1)
            ...
            if self.config.add_features:
                ...
                fused_features = front_image_features + back_image_features + lidar_features
            else:
                fused_features = torch.cat((front_image_features, back_image_features, lidar_features), dim=1)
